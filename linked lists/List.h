...
template <typename NODETYPE> class List
{
  public:
    ...
    void concatenate(List<char>& newlist) {
        
        ListNode<NODETYPE> *currentPtr = newlist.firstPtr;//make a pointer point to the first item in the list
        
        while (currentPtr != nullptr) // while there are still items in the list
        {
            ListNode<NODETYPE>* newPtr = newlist.firstPtr;//other pointer to add values, seperated from currentptr just in case
            
            
            lastPtr->nextPtr = newPtr; // update previous last node
            std::cout << newPtr->data << std::endl;//print value of added node
            lastPtr = newPtr;          // new last node
            if (newlist.firstPtr == newlist.lastPtr) {//if other list is empty, pointers are nullptrs
                newlist.firstPtr = newlist.lastPtr = nullptr;
                std::cout << "done\n";
                currentPtr = nullptr;
            }// no nodes remain after removal
            else {
                newlist.firstPtr = newlist.firstPtr->nextPtr;//step trough newlist 
                std::cout << "next\n";
                currentPtr = currentPtr->nextPtr;
